...
**Шаг 1 — Обновить тесты. ОБЯЗАТЕЛЬНО, без исключений.**
- Найти существующие тесты для изменённых модулей (`grep` по имени функции/роутера)
- Для каждого нового endpoint, функции или компонента — написать хотя бы один тест
- Для изменённого поведения — обновить существующие тесты
- Тесты пишутся ДО запуска `test.sh`. Нельзя перейти к Шагу 2 без выполнения Шага 1.
